...
func TestApplyAgainstPostgres(t *testing.T) {
	...
	t.Run("fresh and repeat", func(t *testing.T) {
		...
		if len(first.Applied) != 7 || len(second.Applied) != 7 ||
...
SELECT count(*)
FROM wanwork_meta.schema_migrations`).Scan(&rows); err != nil || rows != 7 {
...
VALUES (8, 'future', $1)`, strings.Repeat("0", 64)); err != nil {
...
ALTER TABLE wanwork_im.provider_realms
	...
	t.Run("two migrators serialize", func(t *testing.T) {
		...
		workers.Wait()
		for index := range connections {
			if errorsByWorker[index] != nil || len(results[index].Applied) != 7 {
				t.Fatalf(
