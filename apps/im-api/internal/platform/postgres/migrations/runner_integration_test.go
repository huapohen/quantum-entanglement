...
func TestApplyAgainstPostgres(t *testing.T) {
	...
	t.Run("fresh and repeat", func(t *testing.T) {
		...
		if len(first.Applied) != 6 || len(second.Applied) != 6 ||
...
SELECT count(*)
FROM wanwork_meta.schema_migrations`).Scan(&rows); err != nil || rows != 6 {
...
VALUES (7, 'future', $1)`, strings.Repeat("0", 64)); err != nil {
...
ALTER TABLE wanwork_im.provider_realms
	...
	t.Run("two migrators serialize", func(t *testing.T) {
		...
		workers.Wait()
		for index := range connections {
			if errorsByWorker[index] != nil || len(results[index].Applied) != 6 {
				t.Fatalf(
