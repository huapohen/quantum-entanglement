...
5. **Todos**：真实走通 goal → Chief 拆解 → plan review → parallel branches → AI/human review → merge，记录每一步的状态与失败恢复。
6. **Raft/FloatIM**：注册体验群聊、Agent 邀请、临时组队、人工 check-in 和本地 daemon；观察群消息与任务/产物是否有结构化关联。
7. **Mindra**：申请 security/trust 文档，核验 RBAC 资源粒度、审计导出、审批回读和“3,000+ 集成”的实现类型。
8. **CodexLoom**：运行仓库测试与最小多 Agent demo，重点验证 message delivery recovery、Artifact handoff、Needs You 与外部发送之间的授权边界。

### P2：持续追踪

9. **YouMind**：主站恢复后记录 Board、引用、成品版本和 Agent 身份；确认中文域名与主域法律/账号关系。
10. **Gotaa Pi.Agent**：只在出现新的官方域名、公司公告或仓库后重启研究；不要围绕停放域名继续猜测。
