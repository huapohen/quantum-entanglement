...
> 当前代码 HEAD：`d5e91c1`（实现节点为 `36cd0b4`，race 矩阵为 `025b5c7`）
...
8. 子进程在纯 handler 中被 SIGKILL 后，父进程重开 store 只能 expiry recovery 为
   `FAILED/EXPIRED`，不会出现 result acceptance；
9. heartbeat/expiry 在两个 SQLite connection 上竞争时，最终只可能是：
...
10. 两个 connection 同时 relinquish 时恰好一个返回 `True`、另一个返回 `False`，持久状态只有
   一个完整 terminal fence；
11. scoped start event 的 stream version 不因 heartbeat/relinquish 改写；
12. 重复 relinquish、失效 claim、非法 limit 和非法输入均 fail closed。
