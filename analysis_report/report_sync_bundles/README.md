...
| 文件 | 状态 | 本地库存口径 | 说明 |
|---|---|---|---|
| `checkpoint-20260827-clawith-delivery-blueprint.json` | `current` | 41 source、42 source-target、27 images | Scoped atomic start/worker authority 既有边界，加上 Clawith 部门样板、运行时/发布源码复核和第 26 张官网证据后的 schema v3 库存；已对生成内容执行 `--verify` |
...
证据失效。四个旧 JSON 必须保持原样：不得覆盖、删除、重命名或手工修改。后续每个阶段继续使用
