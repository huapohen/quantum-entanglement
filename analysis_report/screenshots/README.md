...
`manifest.json` 当前索引 31 张图片，并固定每个文件的 SHA-256、字节数、像素尺寸、来源
类型、内容范围、派生关系与访问分类。
...
- 前十张文件是未脱敏的受限原件，可能包含姓名、头像、侧栏、账号水印或内部页面结构；
  ...
  同卡片口径冲突。最后四项是 Topic 33 与 Topic 34 两个 W2 PostgreSQL 检查点图各自的 source
  SVG 与 rsvg 派生 PNG；它们是报告导航图，不是独立运行证据。
