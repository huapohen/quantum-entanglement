...
| [`research/43_event_projection_contract_checkpoint.md`](research/43_event_projection_contract_checkpoint.md) | 当前 W2 本地增量：`local_pending`，`5e9894a` | 框架中立的 global event projection replay contract、精确 scope、opaque cursor、page-level CAS checkpoint、at-least-once 重放、故障映射与单测证据；尚未关闭 PostgreSQL durable checkpoint/inbox-outbox/provider/备份与 result authority 门禁 |
| [`research/44_postgres_projection_checkpoint_implementation.md`](research/44_postgres_projection_checkpoint_implementation.md) | 当前 W2 本地增量：`local_pending`，`4b19975` | PostgreSQL 0008 durable projection checkpoint 表/SECURITY DEFINER CAS 函数、runtime-only authority、重启读回、stale conflict、公开错误映射与 PG18.6 集成证据；全量 Go test/race/vet 已通过；尚未关闭 inbox-outbox/dead-letter/commit-unknown/provider/备份与 result authority 门禁 |
| [`research/45_postgres_native_im_inbox_implementation.md`](research/45_postgres_native_im_inbox_implementation.md) | 当前 W2 本地增量：`local_pending`，`5248a0a` | digest-bound native IM inbox 合同、PostgreSQL 0009、RLS/function-only admission、runtime adapter、insert/replay/conflict、跨租户隔离与 PG18.6 证据；尚未关闭 verified envelope/provider auth、event bridge、outbox/action receipt、dead-letter、reconcile 与真实 provider 门禁 |
| [`research/46_postgres_inbox_commit_unknown_reconcile.md`](research/46_postgres_inbox_commit_unknown_reconcile.md) | 当前 W2 本地增量：`local_pending` | inbox commit-unknown 隔离、fresh connection 精确 readback、replayed/unknown/integrity 错误边界；尚未关闭 callback authenticity、outbox/action receipt 与真实 provider 门禁 |
| [`research/47_postgres_atomic_inbox_event_bridge.md`](research/47_postgres_atomic_inbox_event_bridge.md) | 当前 W2 本地增量：`local_pending`，`f24786a` | verified inbox 与 canonical event 在单一 PostgreSQL transaction 内原子 admission/append、双 readback 和 unknown reconcile；尚未关闭 provider callback、outbox/dead-letter 与 action authority |
| [`research/48_postgres_inbox_semantics_hardening.md`](research/48_postgres_inbox_semantics_hardening.md) | 当前 W2 本地增量：`local_pending`，`10325d4` | migration 0010 强化 identity、payload digest、reference shape、全零摘要和 delivery counter 边界，保留 0009 checksum 并分代验证 v9/v10 function digest |
| [`research/49_w2_postgres_stage_local_closure.md`](research/49_w2_postgres_stage_local_closure.md) | 当前阶段本地收口：`local_pending`，`c2e3266` | v10 authority/runtime 测试夹具对齐、fresh readback deadline race 修复；PG18.6 normal/race 25 package、各 1,444 test pass、vet 通过；Notion 延后批量同步 |
| [`docs/production/evidence/checkpoints/w2-postgres-c2e3266.local-run.json`](../docs/production/evidence/checkpoints/w2-postgres-c2e3266.local-run.json) | 机器可读本地验证摘要：`local_pending` | exact source/tree、normal/race JSON log digest、25 package/1,444 test pass、0 skip/fail、PostgreSQL 18.6 disposable loopback 限制 |
