...
| RQ-003 | **external identity、稳定 Actor、tenant membership、conversation membership、acting principal/on-behalf-of 与 workload/delegation 必须分层。** `_portfolio/master_research_report.md:47,673-687,909-921`; `clawith/research_report.md:543-557`; `tech-agent-security-governance/research_report.md:189-231` | W1 已冻结 `ProviderRealmID/ExternalIdentityRef/ActorRef/ActorSnapshot`；W2/W4 增加 `HumanPrincipal/WorkloadPrincipal/TenantMembership/ConversationMembership/ActingContext/OnBehalfOf/DelegationGrant/CapabilityLease`。Clerk 只认证 human，平台逐层做 membership/authorization。 | provider ref/prefix/type 都不授予权限；realm/app 隔离；委托只能缩权；task/audience/purpose/TTL/PoP 绑定；离职、取消、撤销使新 lease 失效。 | W1 值完成；W2-W4 authority | 同一 external human 可进入两个 tenant 但权限不同；同一 provider subject 跨 realm 不串号；receipt 还原 human→Agent→workload→tool；无 membership、伪造 service/Agent 或请求父级权限均失败。 |
| RQ-004 | **持久 Agent 身份、不可变 release、tenant installation 与 runtime session 解耦，并可 offboard。** `_portfolio/master_research_report.md:254-262,1031-1038,1185`; `clawith/research_report.md:148-152`; `raft-slock/research_report.md:209-230`; `agentteams/research_report.md:113-123,192-201` | `ActorRef` 已与 snapshot revision 分离；W2 分离 `AgentDefinition/Release(digest)/Installation/RuntimeIncarnation`，提供 suspend/offboard/reassign API。SemVer 只作显示/兼容标签。 | release bytes/digest 不可漂移；Run/Message pin release；回收 membership、claim、schedule、delegation、credential、provider projection；Memory/Artifact 按 retention 处理。 | W1 stable ref；W2/W4 完整对象 | runtime restart/swap 不换 Actor；升级不改历史 producer；重复 SemVer 不可换 bytes；offboard 后不能读新增内容或执行，历史 Artifact 责任链保留。 |
| RQ-005 | **Agent-native IM 需要一等 Agent 身份、独立 membership 与群级触发规则。** `floatim-floatboat/research_report.md:152-163,229-240`; `clawith/research_report.md:198-226` | 人与 Agent 共享 `ActorRef/Participant/ConversationMembership/MentionIntent`；Agent 是融云普通用户；群策略默认 `mention_only`。metadata codec 独立见 RQ-043。 | sender 由平台 binding；system/service 默认不作普通聊天用户；平台校验 tenant、membership、Agent installation/status、policy、budget；主 Agent/service 不冒充目标 Agent。 | W1 actor value；W2-W4 membership/use case | 人和 Agent 使用同一成员 API；非 mention 不触发；无 persisted Actor/active installation/membership 不能发言；UI/事件/审计 actor 一致。 |
| RQ-006 | **`@Agent` 子群是 WorkConversation/Handoff topology，不是 Task、ACL 或 authority。** `clawith/research_report.md:204-226,493-503`; `_portfolio/master_research_report.md:220-226`; `codexloom/research_report.md:216-234` | W1 已冻结 `ConversationRef/Snapshot` 与 parent/root/invocation all-or-none；W4 实现 `MentionAdmission/Invocation/WorkConversation/HandoffContract`，幂等键 `(tenant,parentGroup,rootMessage,agentInstallation)`。 | durable inbox、child 独立 membership/object auth、context cutoff、deadline/cycle/depth/budget guard；父群默认不继承子群 ACL；conversation 变化触发 held-draft action-time 重验。 | W1 topology；W3/W4 use case | duplicate/reorder/resume 只产生一个 Task/子群/Invocation；父群未显式加入不能读 child；Agent 只在子群输出，父群仅有受限卡片和人工授权发布的 Artifact ref。 |
...
| RQ-008 | **Everything-is-a-plugin 是受不变量约束的 capability seam。** `deepseek-harness/research_report.md:23-32,207-216,236-251,516-519,568-579`; `_portfolio/master_research_report.md:338-348` | `PluginManifest/Definition/Instance/ProviderBinding/ConsumerPort`；确定性 discover→validate→configure→start→ready→drain→stop；终态统一为 stopped/failed，最终资源清理由 host effect registry 持有。 | package trust/approval 与 manifest 分离；依赖 DAG/循环拒绝；all-ready barrier、drain 拒绝新工作、逆序清理；Host mutex 外执行 plugin callback，starting/stopping 单 owner，reentrant/concurrent lifecycle 快速拒绝；当前 goroutine panic 转固定无 payload error，startup 仍 rollback、shutdown 不跳过后续回收；lifecycle duration 只形成 cooperative context deadline，不冒充强杀；插件不能直写核心表或绕过 Action Plane。 | W1 | fake 替换 Clerk/融云后领域测试不变；validate/configure/start/ready/drain/stop 任一点失败及 concurrent/repeat lifecycle 后 route/listener/timer/lease 为零残留；全部 callback 可观察 State 且不死锁，blocked callback 不能阻止状态观察或抢占 owner；各 phase panic canary 不泄漏且后续回收完整；deadline 已过但 callback 未返回时仍保持 owner/state，释放后才收敛。 |
| RQ-009 | **最终组合树才是部署真相。** `deepseek-harness/research_report.md:218-234,404-414`; `agentspace/research_report.md:1852-1861`; `tech-agent-security-governance/research_report.md:239-276` | `Registry builder -> Freeze -> profile -> ordered bundles -> tenant overlay -> EffectiveConfiguration`；相同 row ID 后层整行替换，显式 tombstone 删除；Attempt 保存 source revisions 与 effective digest。 | Freeze 重验并克隆 schema/broker/manifest/package graph，runtime read 在 Freeze 前拒绝、late registration 关闭；同层冲突/跨租户 overlay/prompt/CLI/home patch 拒绝；schema/version/artifact pin；host 重算完整 manifest digest，PackageRecord 以 approved manifest digest + admission revision 绑定批准；Secret raw locator 先经 broker admission，配置只引用 claim digest/revision，Effective/Factory 只保存非 bearer binding view；capability/egress/secret/artifact/schema/binding/manifest/admission diff 交给 host-owned admission，配置不能自证批准。 | W1/W7 | Freeze completeness/tamper/detachment/concurrency race；manifest/config-schema/broker/claim/effective v3 golden；bundle 优先级、整行替换、timeout/admission/Secret retarget diff、跨 scope anti-replay、撤销与 Host 漂移拒绝；首次启动和未批准扩权均阻断自动晋升。 |
| RQ-010 | **Durable event spine 是恢复/审计/流式 UI 的共同底座。** `deepseek-harness/research_report.md:255-312,543-591,823-842`; `sandbase-harness/research_report.md:318-351`; `_portfolio/master_research_report.md:183-190` | W1 已冻结 `EventToAppend/StoredEvent/EventStore` port、append/read page/scoped opaque cursor 与 volatile deterministic fake；W2 实现 PostgreSQL stream/event/checkpoint、production projector 与 backfill+live。 | store 独占 seq/globalPosition/recordedAt；整批 expected revision、ordered strict exact retry/conflict、全有或全无、unknown event 保留、deep snapshot、empty/unknown/contradictory/typed-nil characteristics fail-closed、cooperative context checks；durable/live 与 EventStore/ActionReceipt 分域；正文/secret 用受控 ref。 | W1 port/fake 已完成；W2 durability/projection 未实现 | W1：28 个 event test，覆盖 64 路 CAS/replay、128 stream global order、scope/namespace cursor、失败零写入、fixture backfill no gap/dup；明确不宣称 durable/tamper-evident/Action receipt/Agent replay，checksum 不冒充 MAC。W2：crash/reopen/kill-9/restore、projection 清库重建、subscribe-before-backfill+live 与 source evidence 通过。 |
...
| RQ-028 | **RongCloud 是 transport/projection，不是 business source of truth；adapter 能启动不证明 provider 语义完整。** `openagents/research_report.md:341-363,466-528`; `holaos/research_report.md:595-607`; `agentspace/research_report.md:739-759` | `InboundChannelBinding/ExternalIdentityBinding/ProviderProjection/TransportReceipt/ProviderCapabilities`；平台持有 organization、ACL、admitted message、Task、Artifact。 | webhook authenticate/dedupe、realm mapping、attachment taint、wrong-group/output DLP、kill switch；stable ID、event auth、resume、mention、file/edit/delete、idempotency、acceptance query 分项声明，required capability 缺失则 not-ready。 | W0/W2/W3 | provider history/group 被删除或 metadata 伪造时平台权限不变；fake↔RongCloud adapter 不改变 canonical contract；unverified capability 不得静默模拟，ACK loss 进入 unknown/reconcile。 |
...
| RQ-042 | **可执行 App/Extension/Plugin 与声明式 Skill/Tool definition 是不同信任类。** `kirocrew/research_report.md:31-33`; `pi-agent/research_report.md:34-39,73-79`; `tutti-vm/research_report.md:41-47`; `protocol-mcp/research_report.md:281-287,392-404`; `deepseek-harness/research_report.md:384-414,443-456,516-519,568-581,825-840`; `omnigent/research_report.md:355-366,505-534,570-589,1242-1261`; `sandbase-harness/research_report.md:373-414,1160-1163` | `ExecutablePackageVersion/ExecutionIsolationProfile/RuntimeGrant/ProcessInstance`；W1 已冻结 host-owned version refs、Supervisor IPC、generation/fence、cancel→grace→kill→wait/reap/release receipts 与 volatile non-isolating fake；Plugin Host 只允许 host 编译并准入的内建插件。 | 不可信包不进 API/Gateway/Plugin Host 主进程；API 不持 raw argv/env/host path/secret/runtime socket/process handle；真实 backend 使用独立 UID/container/microVM，限制 fs/network/process/env/secret/resource；kill 不证明 effect 未发生，缺 exit/reap/release 进入 operator-visible quarantine，effectful stop 始终 reconcile；安装 lifecycle script 也属于执行。 | W1 合同/fake 已完成；共享 wire、durable supervisor 与真实 OS isolation/conformance 在 W4/W7 | W1：profile 扩权逐项拒绝、24 路 generation CAS 单 winner、old fence 拒绝、kill 无 wait quarantine、forced release 仍 `dispatched_unknown`、fake 明示 `isolation=none`；W4/W7：恶意包不能读宿主 home/env/其他 tenant/metadata，不能撞垮核心 API，fork/daemon/orphan 可验证回收，撤销/结束后零残留。 |
...
1. 改动 RQ-001 至 RQ-045 任一结论，必须同时修改 PRD/Architecture/contract/test，并单独 commit；
