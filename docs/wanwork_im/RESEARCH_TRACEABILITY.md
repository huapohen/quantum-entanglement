...
| RQ-009 | **最终组合树才是部署真相。** `deepseek-harness/research_report.md:218-234,404-414` | `profile -> ordered bundles -> tenant overlay -> EffectiveConfiguration`；相同 row ID 后层整行替换，显式 tombstone 删除；Attempt 保存 source revisions 与 effective digest。 | 同层冲突/跨租户 overlay/prompt/CLI/home patch 拒绝；schema/version/artifact pin；capability/egress/secret-ref/artifact/schema/binding diff 交给 host-owned admission，配置不能自证批准。 | W1/W7 | golden canonical bytes/digest、bundle 优先级、整行替换、升级 diff、漂移告警；首次启动和未批准扩权均阻断自动晋升。 |
