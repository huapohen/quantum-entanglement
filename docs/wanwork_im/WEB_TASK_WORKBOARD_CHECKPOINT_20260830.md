...
5. 点击“发布引用到父群”：父群只收到 Artifact ID/digest 引用，Artifact 正文继续留在 Workboard。
6. 重复点击或重复请求只返回 `replayed=true`，不会重复创建任务、产物、Needs You 或父群引用。
7. 点击“退回”：Artifact 变为 `rejected`，Task 变为 `rejected`，同样保留审阅轨迹。
...
| 发布已接受引用 | POST | `/api/v1/demo/im/artifacts/:artifactId/publish` |
...
- 只有 `accepted` Artifact 才能发布；父群消息只携带 Artifact ID、digest 和引用类型，不复制正文。
- 发布引用使用确定性的 client message ID，重复发布返回 `replayed=true`，平台消息仍以父群投影为准。
- 当前投影是进程内内存，重启会清空；生产版本必须迁移到 tenant-bound PostgreSQL，接入 revision、
  expected-revision、outbox、crash recovery 和审计证据。
- 当前 Workboard 仍没有文件上传、版本比较和多人并发审阅；这些属于后续 W4/W5 增量。
...
2. 增加 Task recovery、Artifact 版本和多人审阅并发控制。
