...
checkpoint plus the explicit migration-7 activation kernel (latest pushed HEAD `f5e0e4d`). It has a
...
authority, and classifies unusable graphs as `partial`, `drift` or `orphan`. A receipt-bound,
non-emitting reconciliation CAS now closes a complete graph against a still-running owner and is
idempotent on replay. An explicit result-specific backup/restore path binds active migration-7
topology, bytes, geometry and table counts without changing the legacy backup API.
...
counted as a closed Gate A–E. The next release-blocking work is crash/kill publication evidence,
clean-host restore/replay, compatibility policy and the receipt-bound worker gate.
